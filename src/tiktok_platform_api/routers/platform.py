...
@router.post("/integrations/tiktok/sync")
def sync_tiktok(
    request: Request,
    user: object = Depends(require_mutation_auth),
    db: Session = Depends(get_db),
    ...
    token = get_oauth_token(db, "tiktok")
    if token is None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="No TikTok account is connected.")
    ...
    creator_info_refreshed = False

    try:
        refreshed = client.ensure_fresh_token(token)
        ...
        creator_info = client.query_creator_info(token.access_token)
        set_setting(
            db,
            "tiktok_creator_info_cache",
            {"data": creator_info, "fetched_at": utcnow().isoformat(), "last_error": None},
        )
        creator_info_refreshed = True
    except TikTokApiError as exc:
        set_setting(
            db,
            "tiktok_creator_info_cache",
            {"data": {}, "fetched_at": utcnow().isoformat(), "last_error": str(exc)},
        )
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=str(exc)) from exc

    pipeline_state = get_setting(
        db,
        "pipeline",
        {
            "paused": False,
            "upload_mode": settings.upload_mode,
            "target_videos_min": settings.target_videos_min,
            "target_videos_max": settings.target_videos_max,
        },
    )
    log_operator_action(
        db,
        ...
        request=request,
        details={
            "token_refreshed": refreshed_token,
            "creator_info_refreshed": creator_info_refreshed,
            "pipeline_paused": bool(pipeline_state.get("paused")),
        },
    )
    db.commit()
    return {
        ...
        "integration": _serialize_tiktok_status(db, settings),
